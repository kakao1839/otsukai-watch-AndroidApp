...
class BluetoothConnectionService(internal var mContext: Context) {
  ...
  private inner class AcceptThread : Thread() {
    ...
    fun cancel() {
      ...
      try {
        ...
        Log.e(TAG, "AcceptThread cancel: Close of AcceptThread ServerSocket failed. " + e.message)
      }
  ...
  private inner class ConnectThread(device: BluetoothDevice, uuid: UUID) : Thread() {
    ...
    override fun run() {
      ...
      Log.i(TAG, "ConnectThread RUN mConnectThread ")
    ...
    fun cancel() {
      try {
        ...
        Log.e(TAG, "ConnectThread cancel: close() of mmSocket in Connectthread failed. " + e.message)
      }
  ...
  companion object {
    val TAG = "BluetoothConnectionServ"

    val appName = "Bluetooth_Test"

    val MY_UUID_INSECURE = UUID.fromString("7744a5ac-b38a-11e9-a2a3-2a2ae2dbcce4")
  }
